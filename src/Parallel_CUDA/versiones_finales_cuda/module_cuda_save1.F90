...
!     =========================================================
      !----------------Textures--------------------------------
      integer,dimension(:,:), device,target, allocatable :: d_No_cp
      integer,dimension(:,:), device,target, allocatable :: d_No_vp
      real*8, dimension(:),   device,target, allocatable :: d_phiv
      real*8, dimension(:),   device,target, allocatable :: d_phi
      integer,dimension(:,:), device,target, allocatable :: d_Surro
      real*8, dimension(:,:), device,target, allocatable :: d_Weigh
      real*8, dimension(:),   device,target, allocatable :: d_dzT
      real*8, dimension(:),   device,target, allocatable :: d_dzB
      integer,dimension(:),   device,target, allocatable :: d_nbev
      integer,dimension(:),   device,target, allocatable :: d_IndexColor

!     =========================================================
      !----------------Pointers--------------------------------
      integer,texture,pointer :: t_No_cp(:,:)
      integer,texture,pointer :: t_No_vp(:,:)
      real*8, texture,pointer :: t_phiv(:)
      real*8, texture,pointer :: t_phi(:)
      integer,texture,pointer :: t_Surro(:,:)
      real*8, texture,pointer :: t_Weigh(:,:)
      real*8, texture,pointer :: t_dzT(:)
      real*8, texture,pointer :: t_dzB(:)
      integer,texture,pointer :: t_nbev(:)
      integer,texture,pointer :: t_IndexColor(:)

!     =========================================================
      !----------------Managed---------------------------------
      real*8, dimension(:,:), managed, allocatable :: m_erreur_c0P
      real*8, dimension(:,:), managed, allocatable :: m_erreur_c1P
      real*8, dimension(:,:), managed, allocatable :: m_erreur_c2P
      real*8, dimension(:),   managed, allocatable :: m_erreur_AllColors

!     =========================================================
!     Threads and blocks distribution 
      integer :: THREADSPBLOCK_X,THREADSPBLOCK_Y 
      type(dim3) :: grid,gridv,tBlock
      type(dim3) :: gridP,gridI
      type(dim3) :: grid_c0P,grid_c1P,grid_c2P

      integer :: shared_mem
      integer :: N_C0,N_C,N_V,N_V0,MaxNumC
      integer :: midP,midI,Inie_c,Fine_c 
      integer :: Elem_c0,Elem_c1,Elem_c2
      integer :: Tam_erreur_AllColors
      integer :: Elem_grid_c0P,Elem_grid_c1P,Elem_grid_c2P
      integer :: Elem_grid_c0P_c1P
...
!     ============================
!     We need THREADSPBLOCK_X*THREADSPBLOCK_Y as a power of 2
      ...
      if (THREADSPBLOCK_X*THREADSPBLOCK_Y.gt.1024) then
         print*,'Number of Threads per block exceeds 1024,'
...
!     ============================
!     Max number of color is 3
   
      IF (N_COLOR.gt.3) THEN
         print*,'Number of colors exceeds 3,'
         print*,'please correct the mesh.'
         stop
      ENDIF

!     ============================
!     Definition elements by color
...
!     CUDA: Allocate memory
      ...
      allocate(m_erreur_c0P(grid_c0P%x,grid_c0P%y))
      allocate(m_erreur_c1P(grid_c1P%x,grid_c1P%y))
      allocate(m_erreur_c2P(grid_c2P%x,grid_c2P%y))    
      allocate(m_erreur_AllColors(Tam_erreur_AllColors))    
...
!     Deallocate memory (CUDA variables)
      ...
      deallocate(m_erreur_c0P(grid_c0P%x,grid_c0P%y))
      deallocate(m_erreur_c1P(grid_c1P%x,grid_c1P%y))
      deallocate(m_erreur_c2P(grid_c2P%x,grid_c2P%y))
      deallocate(m_erreur_AllColors(Tam_erreur_AllColors))
...
!     |____________________________________|
      ...
      integer, parameter :: relaxSORCUDA = 1.2d0
...
!     d_phiNew calculation      
      ...
      if (idyl.le.midP) then   
         idy=idyl*2;
         idx=t_IndexColor(Inie_c+idxl-1)
         index = idy + (idx-1)*NZ
         jc1 = idy + (t_No_cp(idx,1)-1)*NZ
         jc2 = idy + (t_No_cp(idx,2)-1)*NZ
         jc3 = idy + (t_No_cp(idx,3)-1)*NZ      
         jv1 = idy + (t_No_vp(idx,1)-1)*(NZ-1)
         jv2 = idy + (t_No_vp(idx,2)-1)*(NZ-1)
         jv3 = idy + (t_No_vp(idx,3)-1)*(NZ-1)          
         residu = d_rhs(index)                 & 
                - d_Bm(1,index)*t_phiv(jv1)    &
                - d_Bm(2,index)*t_phiv(jv2)    &
                - d_Bm(3,index)*t_phiv(jv3)    &
                - d_Bm(4,index)*t_phiv(jv1-1)  &
                - d_Bm(5,index)*t_phiv(jv2-1)  &
                - d_Bm(6,index)*t_phiv(jv3-1)  &
                - d_Am(1,index)*t_phi(jc1)     &
                - d_Am(2,index)*t_phi(jc2)     &
                - d_Am(3,index)*t_phi(jc3)     & 
                - d_Am(4,index)*t_phi(index+1) &
                - d_Am(5,index)*t_phi(index-1) & 
                - t_phi(index)      
         d_phi(index) = t_phi(index) + relaxSOR*residu
         s_erreur(s_index) = abs(residu) 
      endif
      call syncthreads();
      if (idyl.le.midI) then
...
!     ______________
      if (s_index.eq.1) then
        !m_erreur_c0P(blockIdx%x, blockIdx%y) = s_erreur(1)
         m_erreur_AllColors((blockIdx%x-1) * gridDim%y + blockIdx%y) = s_erreur(1)
      endif      
...
!     ______________
      if (s_index.eq.1) then
        !m_erreur_c1P(blockIdx%x, blockIdx%y) = s_erreur(1)
         m_erreur_AllColors((blockIdx%x-1) * gridDim%y + blockIdx%y + Elem_grid_c0P)= s_erreur(1)
      endif      
...
!     ______________
      if (s_index.eq.1) then
        !m_erreur_c2P(blockIdx%x, blockIdx%y) = s_erreur(1)
        m_erreur_AllColors((blockIdx%x-1) * gridDim%y + blockIdx%y + Elem_grid_c0P_c1P)= s_erreur(1)
      endif      
...
!      enddo
      ...
      som=0.0d0
      !j=1
      jc1 = idy + (t_Surro(idx,1)-1)*NZ
      som = som + t_Weigh(idx,1)*t_phi(jc1)
      !j=2
      jc1 = idy + (t_Surro(idx,2)-1)*NZ
      som = som + t_Weigh(idx,2)*t_phi(jc1)
      !j=3
      jc1 = idy + (t_Surro(idx,3)-1)*NZ
      som = som + t_Weigh(idx,3)*t_phi(jc1)
      !j=4
      jc1 = idy + (t_Surro(idx,4)-1)*NZ
      som = som + t_Weigh(idx,4)*t_phi(jc1)
      !j=5
      jc1 = idy + (t_Surro(idx,5)-1)*NZ
      som = som + t_Weigh(idx,5)*t_phi(jc1)
      !j=6
      jc1 = idy + (t_Surro(idx,6)-1)*NZ
      som = som + t_Weigh(idx,6)*t_phi(jc1)
      !j=7
      jc1 = idy + (t_Surro(idx,7)-1)*NZ
      som = som + t_Weigh(idx,7)*t_phi(jc1)
      !j=8
      jc1 = idy + (t_Surro(idx,8)-1)*NZ
      som = som + t_Weigh(idx,8)*t_phi(jc1)
      !j=9
      jc1 = idy + (t_Surro(idx,9)-1)*NZ
      som = som + t_Weigh(idx,9)*t_phi(jc1)
...
!     Interpolation       
      ...
      if ((idy.ge.1).and.(idy.le.NZ-1)) then              
         d_phiv(indexv) = t_dzB(idy)*d_phivC(index+1) &
                         +t_dzT(idy)*d_phivC(index)              
      endif                         
!     -------------------------------------------------
