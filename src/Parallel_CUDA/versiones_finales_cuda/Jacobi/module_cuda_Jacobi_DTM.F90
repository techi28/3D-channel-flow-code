...
!     =========================================================
      !----------------Managed------------------------------------
      real*8, dimension(:,:), managed, allocatable :: m_erreur
...
!     CUDA: Allocate memory      
	  ...
	  allocate(m_erreur(grid%x,grid%y))          
#     ifdef KeyMSOR_cuda           
...
!     Deallocate memory (CUDA variables)
	  ...
	  deallocate(m_erreur(grid%x,grid%y))            
#     ifdef KeyMSOR_cuda           
...
!        Using Texture Memory         
		 ...
		 residu = d_rhs(index)                 & 
                ...
                - d_phi(index) 
         d_phiNew(index) = t_phi(index) + relaxSOR*residu
...
!     ______________
      if (s_index.eq.1) then
        m_erreur(blockIdx%x, blockIdx%y) = s_erreur(1)
      endif      
...
!	  som=0.0d0  !Faltó inicializar som
!      do j=1,MaxNumC
!         !jc1 = idy + (d_Surro(idx,j)-1)*NZ
!         !som = som + d_Weigh(idx,j)*d_phi(jc1)
!         jc1 = idy + (t_Surro(idx,j)-1)*NZ
!         som = som + t_Weigh(idx,j)*t_phi(jc1)
!      enddo
