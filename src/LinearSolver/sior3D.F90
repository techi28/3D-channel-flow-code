...
!---------------------------------------------------------------------!
!            JACOBI FOR THE SOLUTION OF THE 3D POISSON EQUATION       !
!                            July 2014                                !
!---------------------------------------------------------------------!
!wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww!
...
!     --------------------------------------
      ...
      integer :: iter,Display,SaveEpsResults
!     ----------------------------------------
      integer :: s,ii,jj,ivert
      real*8  :: nnx,nny,nnz,x,y,z
      real*8  :: dfBdn,Neumanndfdn3D
!     ---------------------------------------- 
      real :: start,finish,startT,finishT
      real :: timeTotal,timeOther,timeBCcc,timeBCvv,timeComm,timeInter,timeLoops
!     ----------------------------------------
...
!     ----------------------------------------
      real*8,dimension(:,:,:) :: vecA(N_CELL,NZ,12)
      real*8,dimension(:) :: vecB(12)
!     ----------------------------------------
!     ====================================
!     ======    PARALLEL DISPLAYS  =======
#     ifdef KeyParallel
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#     ifdef KeyDbg
         write(*,'(t15,60a)'), '----> Begin subroutine: Jacobi 3D'
#     endif
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!    |________________________________________________________|
      ...
      do k=1,NZ
         do i=1,N_CELL0 	
            ...
            !vecA(i,k,1)  = Bmv1T(i,k)
            !vecA(i,k,2)  = Bmv2T(i,k)
            !vecA(i,k,3)  = Bmv3T(i,k)
            !vecA(i,k,4)  = Bmv1B(i,k) 
            !vecA(i,k,5)  = Bmv2B(i,k)
            !vecA(i,k,6)  = Bmv3B(i,k)
            !vecA(i,k,7)  = Am1(i,k)   
            !vecA(i,k,8)  = Am2(i,k)   
            !vecA(i,k,9)  = Am3(i,k) 
            !vecA(i,k,10) = AmT(i,k)   
...
!     |________________________________________________________|
      ...
      do k=2,NZ-1
         do i=1,N_CELL0      
            !vecB(1)  = phiv(No_vp(i,1),k)
            !vecB(2)  = phiv(No_vp(i,2),k)
            !vecB(3)  = phiv(No_vp(i,3),k)
            !vecB(4)  = phiv(No_vp(i,1),k-1) 
            !vecB(5)  = phiv(No_vp(i,2),k-1)
            !vecB(6)  = phiv(No_vp(i,3),k-1)
            !vecB(7)  = phi(No_cp(i,1),k)   
            !vecB(8)  = phi(No_cp(i,2),k)   
            !vecB(9)  = phi(No_cp(i,3),k)  
            !vecB(10) = phi(i,k+1)   
            !vecB(11) = phi(i,k-1) 
            !vecB(12) = phi(i,k)             
            !residu = rhs(i,k)- dot_product(vecA(i,j,1:12),vecB)
...
!     Update phi using phiNew                
      ...
      enddo
      call cpu_time(finish) 
      timeLoops = timeLoops + (finish-start)
!     ________________________________________________________
...
#     ifndef KeyParallel
      ...
      print*,' '
      print*,'        ------------------------------------'
      print*,'        Distribution of the time in Jacobi'
      print*,'        ------------------------------------'
...
#     ifdef KeyParallel
      ...
      call MPI_ALLGATHER(timeLoops,1,MPI_FLOAT,timeLoopsV,1,MPI_FLOAT,comm3D,code)
      ...
      timeLoopsA = sum(timeLoopsV)/Nprocs
      timeCommA  = sum(timeCommV)/Nprocs      
      timeTotalA = sum(timeTotalV)/Nprocs            
      IF (Display.eq.1) THEN 
      write(*,*),'                 Distribution of the time in Jacobi'
      write(*,*),'--------------------------------------------------------------------------- '
      write(*,*)'  p |  Interpo. |   BC_Cell | BC_Vertex |    Other  |   Loops   |   Comm.   |   Total   '
      write(*,*)'----|-----------|-----------|-----------|-----------|-----------|-----------|-----------'      
      do s=1,Nprocs
         write(*,19),s-1,' |',timeInterV(s),' |',timeBCccV(s),' |',timeBCvvV(s),' |',&
                    timeOtherV(s),' |',timeLoopsV(s),' |',timeCommV(s),' |',timeTotalV(s)
      enddo
      19 format(i4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4)      
      write(*,*)'----|-----------|-----------|-----------|-----------|-----------|-----------|-----------'
      write(*,18),' Avg |',timeInterA,' |',timeBCccA,' |',timeBCvvA,' |',&
                    timeOtherA,' |',timeLoopsA,' |',timeCommA,' |',timeTotalA
      18 format(a6,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4,a2,f10.4)
      write(*,*), ' '
      ENDIF     
#     endif
!     =============== END ================    
...
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#     ifdef KeyDbg
         write(*,'(t15,60a)'), '<---- End   subroutine: Jacobi 3D'
#     endif
!     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
