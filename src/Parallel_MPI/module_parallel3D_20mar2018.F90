...
!     ------------------------------
      ...
      integer :: type_bloc1,type_continuousC
...
!          GLOBAL ASSIGNATION
        ...
        !      call MPI_SEND(ar(1,1),1,type_VERTsend, &
        !                    0,etiquette,comm3D,code)
        !   elseif (rang_topo.eq.0) then
        !      call MPI_RECV(ar(1,1),1,type_VERTsend, &
        !                    s,etiquette,comm3D,statut,code)
        !       !------           
        !       Iini = DimVVdom(s+1,2)
        !       Ifin = DimVVdom(s+1,3)
        !       nv = 0
...
!      _______________________________________________________________ 
!     |      |                                                        |
!     | 4.1.2|  Building the global matrix (vertices) - Version 2016  |
!     |______|________________________________________________________|
